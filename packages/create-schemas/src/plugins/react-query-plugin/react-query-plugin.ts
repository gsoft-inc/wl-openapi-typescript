...
export function reactQueryPlugin(): Plugin {
    return {
        name: "react-query-plugin",
        async buildStart({ document, emitFile }) {
            const { ast, hasMutations } = documentToClient(document);
            const types = documentToTypeNodes(document);
            const baseClientPath = "\"@workleap/create-schemas/plugins/react-query-plugin/openapi-client\"";
            ...
            emitFile({
                ...
                code: [
                    ...
                    hasMutations
                        ...
                        : "import { useQuery, useSuspenseQuery, type UseQueryResult, type UseQueryOptions, type UseSuspenseQueryResult, type UseSuspenseQueryOptions, type QueryKey, type QueryClient, type FetchQueryOptions } from \"@tanstack/react-query\";\n",
                    `import { OpenAPIClient, internal_fetch } from ${baseClientPath};\n\n`,
                    "export { OpenAPIClient };\n\n",
                    JSONSchema.printAST(ast),
...
function documentToClient(document: OpenAPIDocument) {
    const ast: ts.Node[] = [];
    let hasMutations = false;

    if (!document.paths) {
        return { ast, hasMutations };
    }


    // ===== Context =====
    const contextName = capitalize(JSONSchema.toSafeName(document.info.title + "Client"));
